...
DEFINITIONS = {
    ...
    "filesystem.read": (PathInput, Risk.SAFE, "Read a UTF-8 workspace file (up to 1 MB)."),
    "filesystem.write": (WriteInput, Risk.CAUTION, "Create a file; overwrites require approval."),
...
class ToolRegistry:
    ...
    async def execute(
        self, name: str, arguments: dict, *, approved: bool = False, timeout: float = 30
    ) -> ToolResult:
        call_id = uuid4().hex
        ...
        try:
            ...
            result = ToolResult(call_id=call_id, tool=name, status="error", error="timeout")
    ...
    async def _run(self, name, args, target, timeout, approved):
        ...
        if name == "filesystem.write":
            # No implicit parent creation, and exclusive create prevents an overwrite race.
            if target.exists():
